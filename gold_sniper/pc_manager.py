...
from utils.discord_commands import (
    LIFECYCLE_COMMANDS,
    discord_command_authorization_failure,
    format_help_text,
    normalize_command,
)
...
def _enqueue_command(content: str, user_id: int, guild_id: int = 0, channel_id: int = 0) -> None:
...
class PCManagerBot(discord.Client):
    ...
    def _authorized(self, message: discord.Message, cmd: str) -> bool:
        ...
        if failure:
            logging.warning(
                "Discord command refused: cmd=%s reason=%s user=%s guild=%s channel=%s",
                cmd,
                failure,
                message.author.id,
                guild_id,
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if not self._authorized(message, cmd):
            return
        ...
        if is_gold_sniper_running():
            guild_id = int(getattr(getattr(message, "guild", None), "id", 0) or 0)
            channel_id = int(getattr(message.channel, "id", 0) or 0)
            _enqueue_command(normalized, message.author.id, guild_id, channel_id)
            try:
    ...
    async def on_interaction(self, interaction: discord.Interaction) -> None:
        ...
        if not custom_id:
            return
        cmd = "pause" if custom_id == "gs_pause" else "kill" if custom_id == "gs_kill" else ""
        if not cmd:
            return
        guild_id = int(getattr(interaction, "guild_id", 0) or getattr(getattr(interaction, "guild", None), "id", 0) or 0)
        channel_id = int(
            getattr(interaction, "channel_id", 0) or getattr(getattr(interaction, "channel", None), "id", 0) or 0
        )
        failure = discord_command_authorization_failure(
            cmd,
        ...
        if failure:
            logging.warning(
                "Discord interaction refused: cmd=%s reason=%s user=%s guild=%s channel=%s",
                cmd,
                failure,
                interaction.user.id,
                guild_id,
            ...
            await interaction.response.send_message("Commande refusee par garde Discord.", ephemeral=True)
            return
        if custom_id == "gs_pause":
            if is_gold_sniper_running():
                _enqueue_command("!pause", interaction.user.id, guild_id, channel_id)
