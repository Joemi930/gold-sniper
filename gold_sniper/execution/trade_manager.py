...
def _protected_sl_after_tp1(trade: dict[str, Any]) -> float | None:
    entry = float(trade.get("entry_price") or 0.0)
    original_sl = float(trade.get("original_sl") or 0.0)
    direction = str(trade.get("type", "")).upper()
    if direction == "BUY":
        initial_risk = entry - original_sl
        if initial_risk <= 0:
            return None
        return entry + BE_PLUS_RR * initial_risk
    if direction == "SELL":
        initial_risk = original_sl - entry
        if initial_risk <= 0:
            return None
        return entry - BE_PLUS_RR * initial_risk
    return None
...
class TradeManager(BaseAgent):
    def __init__(self, blackboard):
        ...
        self.broker_gateway = BrokerGateway(blackboard)
    ...
    async def _ensure_broker_action_allowed(
        ...
        *,
        ...
        decision = self.broker_gateway.guard.can_send_broker_order(action, context=context)
        if decision.allowed:
            return True

        self.logger.warning(
            "Action broker refusee avant execution | "
    ...
    async def place_order(self, signal_data: Dict[str, Any]) -> bool:
        ...
        """
        if not await self._ensure_broker_action_allowed(
            BrokerAction.OPEN_ORDER,
            context={"source": "place_order"},
            clear_signal=True,
        ):
            return False
        ...
        # Requete atomique: entree + SL + TP broker dans la meme demande.
        ...
        result = await self.broker_gateway.send_order(
            BrokerAction.OPEN_ORDER,
            request,
            context={"source": "place_order", "signal": action},
        )
            ...
            if not trade.get("partial_closed", False):
                ...
                if tp1_reached:
                    ...
                    if close_price > 0:
                        ...
                        close_res = await self.broker_gateway.send_order(
                            BrokerAction.PARTIAL_CLOSE,
                            close_req,
                            context={"source": "manage_active_trades", "ticket": ticket},
                        )
                        if getattr(close_res, "allowed", True) is False:
                            continue
                    ...
                    if partial_ok:
                        protected_sl = _protected_sl_after_tp1(trade)
                        if protected_sl is None:
                            protected_sl = float(entry)
                            self.logger.warning(
                                f"Risque initial invalide sur {ticket}: fallback BreakEven strict."
                            )
                        be_req = {
                            "action": mt5.TRADE_ACTION_SLTP,
                            "position": ticket,
                            "symbol": SYMBOL,
                            "sl": float(protected_sl),
                            "tp": float(tp),
                        }
                        res = await self.broker_gateway.send_order(
                            BrokerAction.MODIFY_SLTP,
                            be_req,
                            context={"source": "manage_active_trades", "ticket": ticket, "reason": "BE_PLUS"},
                        )
                        if getattr(res, "allowed", True) is False:
                            continue
                        if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                            self.logger.trade(f"BE+ active sur ticket {ticket} | SL={protected_sl:.2f}")
                            async with self.blackboard._lock:
                                t = self.blackboard._data["active_trades"].get(ticket)
                                if t:
                                    t["breakeven_activated"] = True
                                    t["be_plus_activated"] = True
                                    t["be_plus_rr"] = BE_PLUS_RR
                                    t["protected_sl"] = float(protected_sl)
                                    t["partial_closed"] = True
                                    t["current_sl"] = float(protected_sl)
    ...
    async def _modify_sl(self, ticket: int, new_sl: float, tp: float, trade: dict, reason: str) -> None:
        ...
        res = await self.broker_gateway.send_order(
            BrokerAction.TRAILING_STOP,
            req,
            context={"source": "_modify_sl", "ticket": ticket, "reason": reason},
        )
        if getattr(res, "allowed", True) is False:
            return
        if res and res.retcode == mt5.TRADE_RETCODE_DONE:
