...
async def _close_if_gap_breached(position, blackboard) -> bool:
    ...
    close_result = await _close_gap_position(position, current_price, blackboard)
...
async def _close_gap_position(position, close_price: float, blackboard):
    close_type = (
        mt5.ORDER_TYPE_SELL
    ...
    return await BrokerGateway(blackboard).send_order(
        BrokerAction.RECOVERY_CLOSE,
        request,
        context={"source": "recovery_manager", "ticket": int(position.ticket)},
    )
