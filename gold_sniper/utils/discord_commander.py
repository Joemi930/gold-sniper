...
class DiscordCommander:
        ...
        try:
            with inbox_lock(self._inbox_lock_path):
                ...
                for line in lines:
                    ...
                    if failure:
                        logger.warning(
                            "Discord inbox command refused: cmd=%s reason=%s user=%s",
                            cmd,
                            failure,
                            user_id,
                        )
    ...
    async def handle_command(self, cmd: str, args: list[str], raw: str = "") -> None:
        failure = discord_command_authorization_failure(
            ...
            channel_id=config.DISCORD_COMMANDS_CHANNEL,
            config_module=config,
        )
        if failure:
            logger.warning("Discord command refused before handler: cmd=%s reason=%s", cmd, failure)
            await self._reply_embed("Commande refusee", "Configuration Discord commandes incomplete.", COLOR_RED)
            return
