...
from config import MAX_RISK_PCT_PER_TRADE, RISK_PCT_PER_TRADE, SYMBOL
...
def calculate_dynamic_lot(
    ...
    configured_risk_pct = min(float(risk_pct), float(MAX_RISK_PCT_PER_TRADE))
    effective_risk_pct = min(
        configured_risk_pct * float(risk_modifier),
        float(MAX_RISK_PCT_PER_TRADE),
    )
