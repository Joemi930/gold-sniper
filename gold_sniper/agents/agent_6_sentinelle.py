        ...
        return AgentResult(
            ...
            payload={
                **state,
                "shadow_ict_contract": {
                    "agent_id": "agent_6",
                    "observations": [state["reason"]],
                    "score": state["score"],
                    "confidence": 0.0 if state["veto"] else 1.0,
                    "hard_veto": state["veto"],
                    "reason": state["reason"],
                    "uncertainty": "LOW",
                    "alternative_scenario": {"scenario": "WAIT_NEWS_CLEARED", "condition": "BLACKOUT_ENDED"} if state["blocked"] else {"scenario": "NONE", "condition": "NONE"},
                    "contextual_notes": {
                        "pre_news_lockout": state["blocked"] and not state["veto"],
                        "during_news": state["veto"],
                        "post_news_normalized": state.get("stealth_mode", False),
                        "news_hard_veto": state["veto"]
                    },
...
def build_agent_6_observation(result):
    ...
    if result is None:
        return AgentObservation(
            ...
            missing_evidence=["AGENT_6_RESULT_MISSING"],
        )

    payload = _p1_safe_dict(result.payload)
    impact = str(payload.get("impact_level") or "UNKNOWN").upper()
    blocked = bool(payload.get("blocked", False))
    veto = bool(payload.get("veto", False))
    stealth = bool(payload.get("stealth_mode", False))
    feed_alive = bool(payload.get("feed_alive", True))

    missing = []
    warnings = []
    if impact == "UNKNOWN":
        missing.append("NEWS_IMPACT_UNKNOWN")
    if not feed_alive:
        warnings.append("NEWS_FEED_NOT_ALIVE")
    ...
    return AgentObservation(
        ...
        payload={
            "schema_version": "p1.agent_observation.v1",
            "agent_id": "agent_6",
            "status": "OK" if feed_alive else "PARTIAL",
            "news_clear": not blocked and not veto and not stealth,
            "high_impact_window": bool(veto and impact == "HIGH"),
            "post_news_stealth": stealth,
            "medium_impact_nearby": bool(blocked and impact == "MEDIUM"),
            "impact_level": impact,
            "feed_alive": feed_alive,
            "calendar_source": payload.get("calendar_source", "UNKNOWN"),
